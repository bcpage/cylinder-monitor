// processor.js — AudioWorkletProcessor v6
// Additive combined signal, ring buffer, FFT gate, spike-only Pyodide calls.
// Snapshot mode for calibration tab keypress capture.
...
class CylinderProcessor extends AudioWorkletProcessor {
    ...
    this.port.onmessage = (e) => {
      ...
      if (e.data.type === 'snapshot') {
        this.port.postMessage({
          type: 'snapshot', rms: this.lastRms,
          hfEnergy: this.lastHfEnergy, timestamp: currentTime * 1000,
        });
      }
    };
  }
  ...
  process(inputs) {
    ...
    const ch0 = input[0], ch1 = input[1] ?? ch0;
    ...
    while (this.buffer.length >= this.chunkSamples) {
      ...
      if (this.threshold !== null && rms >= this.threshold) {
        ...
        this.port.postMessage({
          type: 'spike', rms, hfEnergy, timestamp: nowMs,
